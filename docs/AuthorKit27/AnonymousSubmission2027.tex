...
\begin{abstract}
% Abstract will be written after the main sections are fixed.
\end{abstract}
...
This paper studies a narrower but important question: when the world model is tokenized, is the decoded full-reference reward the right signal for GRPO, especially in multi-step rollouts? We find two coupled failure modes. First, post-decode verifiers compare candidates with raw ground truth even though the ground truth is not exactly reachable through the tokenizer-decoder pair. Second, multi-step GRPO often collapses a rollout into one sequence-level advantage, even though video prediction is autoregressive and frame errors propagate across horizons. These failures are easy to miss because the training loop remains verifiable: every candidate is still compared with a known target.

The first failure mode is verifier-side. A lossy visual tokenizer imposes an irreducible encode-decode residual on the ground-truth frame. We call this residual the reconstruction floor. It is not caused by any sampled candidate, but post-decode metrics such as MSE, SSIM, and LPIPS still include it when scoring decoded predictions \citep{ssim,lpips}. For ordinary evaluation, this is a reconstruction-quality limit of the tokenizer \citep{vqvae,vqgan,perceptiondistortion}. For GRPO, it is a ranking problem: the update depends on relative rewards within a sample group, so reconstruction-induced noise can change which candidate receives positive advantage.
...
Our contributions are threefold. First, we identify a reconstruction-floor failure mode in tokenized video GRPO and verify that the resulting pairwise rank flips follow the predicted $\arccos(\rho)/\pi$ relationship across several reward spaces. Second, we introduce reconstruction-calibrated verifiers as the reward substrate for both single-step and multi-step training. Third, we introduce temporal-return GRPO for multi-step video rollouts and show, under a fixed held-out protocol on RT-1 prediction, that it improves full-reference fidelity over sequence-level GRPO, with gains increasing toward later horizons. We also report negative auxiliary results for generic GRPO replacements, which supports the conclusion that the effective intervention point is video-structured calibration and temporal credit rather than a generic optimizer swap.

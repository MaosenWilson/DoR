...
%
% Math support (standard, not forbidden by the AAAI kit)
\usepackage{amsmath}
\usepackage{amssymb}
...
\title{Mind the Reconstruction Floor: Calibrating Verifiable Rewards for Tokenized Video World Models}
\author{
    %Authors
...
Tokenized autoregressive models have become a standard recipe for video world models: an image tokenizer maps each frame to discrete codes, and a Transformer predicts the tokens of the next frame conditioned on past frames and an action \cite{ivideogpt,genie}. Because such models generate by sampling, their predictions can be sharpened after pre-training with reinforcement learning from verifiable rewards (RLVR), the paradigm behind recent reasoning models \cite{grpo,deepseekr1}. RLVR-World \cite{rlvrworld} brings this to world models: for each transition it samples a group of candidate next-frame token sequences, decodes them to pixels, scores each candidate against the ground-truth frame with a perceptual metric, and updates the policy with Group Relative Policy Optimization (GRPO) \cite{grpo}. The reward is called \emph{verifiable} because it is computed against a known ground truth rather than a learned critic, and this is taken to make it trustworthy.

We show that in the perceptual domain this trust is misplaced, because the reward is read off \emph{after} decoding. A lossy tokenizer cannot reproduce a frame exactly even from its own encoding, so the decoded reference carries a prediction-independent error that we call the \emph{reward noise floor},
\[
  \phi_{\text{tok}} \;=\; \mathbb{E}\big[\, d\big(\mathrm{decode}(\mathrm{encode}(s')),\, s'\big)\,\big],
\]
the irreducible distance, under a metric $d$, between a frame $s'$ and its tokenizer round-trip. That reconstruction quality upper-bounds generation quality is well known for discrete tokenizers \cite{vqvae,vqgan,fsq}; its consequence for the \emph{reward} has been overlooked. Measuring $\phi_{\text{tok}}$ on robot-manipulation transitions, we find that it exceeds the frame-to-frame dynamics signal under exactly the perceptual and structural metrics RLVR relies on---LPIPS \cite{lpips} and SSIM \cite{ssim}---while staying below it under raw pixel error. The metric that makes the reward perceptually meaningful is the one in which the floor drowns the signal.

A floor on the reward's absolute value would be harmless if it shifted every candidate equally, but GRPO never sees absolute values. It normalizes rewards within each group by their mean and standard deviation, so only the \emph{within-group ranking} of candidates drives the gradient. Crucially, the candidates in a group are all predictions of the same transition: their spread is precisely the dynamics signal to be learned, whereas the floor adds a near-independent perturbation to each candidate that subtracting the group mean does not cancel. Once the dynamics spread drops below that of the floor, the ranking---and with it the sign of the advantage---is governed by reconstruction noise rather than prediction quality. We make this quantitative with a closed-form bound: the probability that a candidate pair is mis-ordered is $\arccos(\rho)/\pi$, where $\rho$ is the rank correlation between the noisy and the clean reward. Across five seeds the measured flip rate ($0.185$) matches the bound ($0.186$) to within $0.002$, and flips concentrate in low-signal windows (about $30\%$ versus $10\%$). The policy is, demonstrably, optimizing reconstruction noise rather than dynamics.

The mechanism dictates the fix: compute the reward in a space that preserves the within-group ranking of true dynamics quality, which means cancelling the floor rather than tolerating it. We give two instances of this floor-cancellation principle. Before decoding, a candidate and the ground truth pass through the same lossy map, so scoring them in the tokenizer's pre-decode code space removes the floor at its source---a reward with $\phi_{\text{tok}}\!\approx\!0$ and no external network, which we call Decode-off Rewards (DoR). After decoding, the floor cancels if each candidate is compared not with the raw ground truth but with the \emph{achievable} target $\mathrm{decode}(\mathrm{encode}(s'))$. On RT-1 single-step prediction over five seeds, the code-space reward attains the best motion fidelity and even surpasses RLVR-World's own decode-side reward, while floor-cancelling the post-decode LPIPS reward raises motion fidelity by $14\%$ over its raw counterpart at matched pixel quality. Calibrating the reward space, rather than fusing metrics or modifying GRPO, is what recovers dynamics: our fusion and GRPO-side variants do not help, and we report them as honest ablations.

In summary, this paper makes three contributions:
\begin{itemize}
\item We identify and characterize the \emph{reward noise floor} $\phi_{\text{tok}}$ of decode-side verifiable rewards, and show that it is general across metrics and tokenizers rather than an artifact of a single metric.
\item We show that the floor corrupts exactly the within-group ranking that GRPO optimizes, and derive a closed-form rank-flip bound $\arccos(\rho)/\pi$ that matches measurement across seeds, turning an abstract signal-to-noise argument into a measurable diagnostic.
\item We propose calibrating the reward space by within-group rank preservation, realized as floor-cancellation, and validate two instances---a zero-overhead pre-decode code-space reward (DoR) and a post-decode reward aligned to the achievable target---both of which recover the dynamics signal that the raw decode-side reward loses.
\end{itemize}
